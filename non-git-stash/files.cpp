...
// sun_alf: processing of additional files
BOOLEAN HandleAdditionalTextFile( FilesUnitPtr file );
...
void GameInitFiles( )
{
	...
	//#else
	AddFilesToPlayersLog( ENRICO_BACKGROUND, 0,255, NULL, NULL );
	AddFilesToPlayersLog( ADDITIONAL_FILE_0, 0, FFORMAT_ADDITIONAL_TEXT, NULL, NULL );
	//#endif
...
void DisplayFilesList( void )
{
	...
	while((pFilesList))//&&(iCounter < MAX_FILES_LIST_LENGTH))
	{
		...
		if ( pFilesList->ubCode <= LAST_JA2_VANILLA_FILE )
			...
			mprintf( FILES_SENDER_TEXT_X, iScreenHeightOffset + ((iCounter + 9) * BLOCK_HEIGHT) + (iCounter * 2) - 2, L"Add. file name #0" );  //TODO: make name look-up
...
BOOLEAN DisplayFormattedText( void )
{
	...
	{
		LoadEncryptedDataFromFile("BINARYDATA\\Files.edt", sString, FILE_STRING_SIZE * (uiOffSet) * 2, FILE_STRING_SIZE * uiLength * 2);
		AssertMsg( FALSE, "Entered unused branch if (pFilesList->ubFormat < ENRICO_BACKGROUND)" );
	}
...
BOOLEAN HandleAdditionalTextFile( FilesUnitPtr file )
{
	...
	wcscpy(sString, L"OLOLO TROLOLO! Xyu, nu3da, opewki! 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789 0123456789");
	AddStringToFilesList( sString );
	
	wcscpy( sString, L"KONEC" );
	AddStringToFilesList( sString );

	// move through list and display
	FileStringPtr pFileString = pFileStringList;  // set head of list
	UINT16 usPositionOnPage = 0;
//	INT32 iFileLineWidth = FILE_VIEWER_WIDTH;
//	INT32 iFileStartX = FILE_VIEWER_X + 2;
	while ( pFileString )
	{
		wcscpy( sString, pFileString->pString );

		// set up font
		UINT32 uiFont = FILES_TEXT_FONT;
		UINT32 uiFlags = IAN_WRAP_NO_SHADOW;

		// now print it
		usPositionOnPage += (INT32)IanDisplayWrappedString( FILE_VIEWER_X + 4, FILE_VIEWER_Y + usPositionOnPage, FILE_VIEWER_WIDTH, FILE_GAP, uiFont, FILE_TEXT_COLOR, sString, 0, FALSE, uiFlags );
